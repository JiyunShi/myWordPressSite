<?php
/**
 * Jspage functions and definitions
 *
...
if ( ! function_exists( 'jspage_setup' ) ) :
	/**
	 ...
	 */
	function jspage_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on Jspage, use a find and replace
		 * to change 'jspage' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'jspage', get_template_directory() . '/languages' );
		...
		add_theme_support( 'custom-background', apply_filters( 'jspage_custom_background_args', array(
 ...
 */
function jspage_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
...
add_filter( 'wp_calculate_image_sizes', 'jspage_content_image_sizes_attr', 10, 2 );
 ...
 */
function jspage_header_image_tag( $html, $header, $attr ) {
	if ( isset( $attr['sizes'] ) ) {
		$html = str_replace( $attr['sizes'], '100vw', $html );
	}
	return $html;
}
add_filter( 'get_header_image_tag', 'jspage_header_image_tag', 10, 3 );
 ...
 */
function jspage_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'jspage_post_thumbnail_sizes_attr', 10, 3 );
 ...
 */
function jspage_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'jspage' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'jspage' ),
...
add_action( 'widgets_init', 'jspage_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'jspage_scripts' );
